...
class ConnectionSamsung2878(Connection):
    ...
    def load_from_yaml(self, node, connection_base):
        ...
        if node is not None:
            ...
            if CONFIG_DEVICE_CONNECTION_TEMPLATE in params_node:
                ...
                self.logger.error(
                    "ERROR: missing 'connection_template' parameter in connection section"
                )
                return False
            ...
            if connection_base is None:
                if self._cfg.host is None:
                    self.logger.error(
                        "ERROR: missing 'host' parameter in configuration section"
                    )
                    return False
                if self._cfg.token is None or self._cfg.token == "":
                    self.logger.error(
                        "ERROR: missing 'token' parameter in configuration section"
                    )
                    return False
                if self._cfg.duid == None:
                    self.logger.error(
                        "ERROR: missing 'mac' parameter in configuration section"
                    )
                    return False
                ...
                self.logger.info(
                    "Configuration, host: {}:{}".format(self._cfg.host, self._cfg.port)
                )
    ...
    def handle_response_auth_success(self, sslSocket, response):
        ...
        self._cfg.socket = sslSocket
        command = '<Request Type="DeviceState" DUID="{}"></Request>'.format(
            self._cfg.duid
        )
